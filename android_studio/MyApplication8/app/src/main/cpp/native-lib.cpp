...
Java_com_asysbang_myapplication_ZinniaHelper_testZinnia(JNIEnv *env, jobject thiz) {
    ...
    if (!zinnia_recognizer_open(recognizer,
                                "/data/user/0/com.asysbang.myapplication/files/handwriting-zh_CN.model")) {
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_asysbang_myapplication_ZinniaHelper_createRecognizer(JNIEnv *env, jobject thiz) {
    LOGE("========create recognizer");
    recognizer = zinnia_recognizer_new();
    character = zinnia_character_new();
    zinnia_character_set_width(character, 600);
    zinnia_character_set_height(character, 600);
    if (!zinnia_recognizer_open(recognizer,
                                "/data/user/0/com.asysbang.myapplication/files/handwriting-zh_CN.model")) {
        fprintf(stderr, "ERROR1: %s\n", zinnia_recognizer_strerror(recognizer));
        LOGE("========%s", zinnia_recognizer_strerror(recognizer));
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_asysbang_myapplication_ZinniaHelper_addCharacter(JNIEnv *env, jobject thiz, jint m_index,
                                                          jint x, jint y) {
    zinnia_character_add(character, m_index, x, y);
    return 1;
}
extern "C"
JNIEXPORT jint JNICALL
Java_com_asysbang_myapplication_ZinniaHelper_getResult(JNIEnv *env, jobject thiz) {
    zinnia_result_t *result = zinnia_recognizer_classify(recognizer, character, 10);
    size_t i;
    if (!result) {
        LOGE("=====getresult error===%s\t%f", zinnia_recognizer_strerror(recognizer));
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_asysbang_myapplication_ZinniaHelper_destroyRecognizer(JNIEnv *env, jobject thiz) {
...
Java_com_asysbang_myapplication_ZinniaHelper_getResults(JNIEnv *env, jobject thiz) {
    zinnia_result_t *result = zinnia_recognizer_classify(recognizer, character, 8);
    size_t i;
    if (!result) {
        LOGE("=====getresult error===%s\t%f", zinnia_recognizer_strerror(recognizer));
        fprintf(stderr, "%s\n", zinnia_recognizer_strerror(recognizer));
    ...
    size_t count = zinnia_result_size(result);
    jclass stringArrayClass = env->FindClass("java/lang/String");
    jobjectArray returnArray = env->NewObjectArray(count, stringArrayClass, nullptr);
    //LOGE("=====getresult size===%s\t%f", zinnia_result_size(result));
    for (i = 0; i < count; ++i) {
        LOGE("========%s\t%f", zinnia_result_value(result, i), zinnia_result_score(result, i));
        fprintf(stdout, "---------%s\t%f\n",
                zinnia_result_value(result, i),
                zinnia_result_score(result, i));
        jstring str = env->NewStringUTF(zinnia_result_value(result, i));
        env->SetObjectArrayElement(returnArray, i, str);
